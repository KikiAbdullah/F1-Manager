...
const UI = {
  ...
  initRaceWeekend() {
    ...
    titleEl.innerText = `ROUND ${
      currentEvent.round
    }: ${currentEvent.grand_prix.toUpperCase()}`;
    document.getElementById("sim-area")?.classList.add("hidden");
  ...
  showRaceWeekendSummary() {
    ...
    const event = Data.schedules.find(
      (s) => s.round === State.currentRound && s.type === "race_weekend"
    );
    const circuit = event
      ? Data.circuits.find((c) => c.id === event.circuit_id)
      : null;
    ...
    resultTitle.innerText =
      next === "DONE" ? "RACE WEEKEND COMPLETE" : "RACE WEEKEND SCHEDULE";
    ...
    resultTable.innerHTML = `
      <div class="race-session-setup">
        <div class="schedule-detail-grid">
          <span>Grand Prix: <strong>${
            event ? event.grand_prix : "-"
          }</strong></span>
          <span>Circuit: <strong>${circuit ? circuit.name : "-"}</strong></span>
          <span>Race Laps: <strong>${
            circuit ? circuit.laps : "-"
          }</strong></span>
          <span>DRS Zones: <strong>${
            circuit ? circuit.drs_zones : "-"
          }</strong></span>
        </div>
        <div class="track-stats-grid">
          <div class="track-stat">
            <span class="track-stat-label">Tyre Deg</span>
            <div class="stat-bar"><div class="stat-bar-fill" style="width:${tyreImpact}%; background:${
      tyreImpact > 70 ? "#ff4444" : tyreImpact > 40 ? "#ffaa00" : "#00cc66"
    }"></div></div>
            <span class="track-stat-val">${tyreImpact}%</span>
          </div>
          <div class="track-stat">
            <span class="track-stat-label">SC Chance</span>
            <div class="stat-bar"><div class="stat-bar-fill" style="width:${scChance}%; background:${
      scChance > 60 ? "#ff4444" : scChance > 30 ? "#ffaa00" : "#00cc66"
    }"></div></div>
          ...
          <div class="track-stat">
            ...
            <div class="stat-bar"><div class="stat-bar-fill" style="width:${
              trackChars.top_speed_importance || 50
            }%; background:#00ddff"></div></div>
            <span class="track-stat-val">${
              trackChars.top_speed_importance || 50
            }%</span>
          </div>
        </div>
        <div class="race-status-grid">
          <span class="${data.fp ? "done" : ""}">FP: ${
      data.fp ? "✅" : "⬜"
    }</span>
          <span class="${data.q1 ? "done" : ""}">Q1: ${
      data.q1 ? "✅" : "⬜"
    }</span>
          <span class="${data.q2 ? "done" : ""}">Q2: ${
      data.q2 ? "✅" : "⬜"
    }</span>
          <span class="${data.q3 ? "done" : ""}">Q3: ${
      data.q3 ? "✅" : "⬜"
    }</span>
          <span class="${data.race ? "done" : ""}">Race: ${
      data.race ? "✅" : "⬜"
    }</span>
        </div>
  ...
  renderNextScheduleAction(next, data) {
    ...
    if (next === "Q1")
      return `<button class="btn-session-start" onclick="Engine.startConfiguredSession('Q1')">▶ MULAI Q1 (15 Lolos)</button>`;
    if (next === "Q2")
      return `<p class="race-setup-note">Top 15 Q1 bertanding. 10 tercepat ke Q3.</p><button class="btn-session-start" onclick="Engine.startConfiguredSession('Q2')">▶ MULAI Q2</button>`;
    if (next === "Q3")
      return `<p class="race-setup-note">Final shootout! Top 10 memperebutkan pole position.</p><button class="btn-session-start" onclick="Engine.startConfiguredSession('Q3')">▶ MULAI Q3</button>`;
    if (next === "RACE") {
      return `
    ...
    const playerPts =
      playerResults.map((r) => `${r.code}: P${r.pos}`).join(", ") || "N/A";
    return `
      <div class="weekend-reward-box">
        <strong>🏆 Weekend Selesai!</strong>
        <span>Hasil: ${playerPts}</span>
        <span>Hadiah: Rp ${
          rewards?.money ? rewards.money.toLocaleString("id-ID") : 0
        }</span>
      </div>
  ...
  renderCompoundButtons(context) {
    ...
    return compounds
      .map((c) => {
        ...
        const selected =
          context === "start" && (State.raceStartCompound || "medium") === c;
        return `<button class="btn-compound ${
          selected ? "active" : ""
        }" style="border-color:${
          data.color
        }" onclick="UI.selectCompound('${c}', '${context}', this)">${
          data.code
        }</button>`;
      })
      .join("");
  ...
  renderLiveStandings(grid) {
    ...
    // 1. Inisialisasi DOM HANYA SEKALI di awal sesi
    if (!container.dataset.initialized || container.children.length <= 2) {
      container.innerHTML = `<div class="panel-title">LIVE LEADERBOARD</div>
      ...
      grid.forEach((racer) => {
        ...
        row.id = `standing-row-${racer.id}`;
        // Menyimpan elemen ke memori agar tidak perlu document.querySelector berulang kali
        racer.domNodes = {
          row: row,
          ...
          tyre: document.createElement("span"),
        };

        racer.domNodes.pos.className = "standing-pos";
        racer.domNodes.code.className = "standing-code";
        racer.domNodes.code.style.borderLeft = `3px solid ${racer.color}`;
        racer.domNodes.code.textContent = racer.code;
        racer.domNodes.team.className = "standing-name";
        racer.domNodes.team.textContent = racer.teamName || "";
        racer.domNodes.gap.className = "standing-lap-time";
        racer.domNodes.tyre.className = "standing-tyre";

        row.appendChild(racer.domNodes.pos);
        row.appendChild(racer.domNodes.code);
        row.appendChild(racer.domNodes.team);
        row.appendChild(racer.domNodes.gap);
        row.appendChild(racer.domNodes.tyre);
    ...
    // 2. UPDATE DOM: Hanya ubah data teks yang dinamis
    grid.forEach((racer, index) => {
      if (!racer.domNodes) return;
      ...
      // Set class untuk styling
      racer.domNodes.row.className = `standing-item ${
        racer.isPlayer ? "player-standing" : ""
      } ${racer.dnf ? "dnf-standing" : ""}`;

      // Update data teks secara spesifik (jauh lebih cepat daripada innerHTML)
      racer.domNodes.pos.textContent = pos;
      racer.domNodes.gap.textContent = racer.finished ? "FIN" : gap;
      racer.domNodes.tyre.innerHTML = `<span class="tyre-badge" style="background:${compColor}">${
        compound?.code || "?"
      }</span> <span class="tyre-wear-text">${wearPct}%</span>`;

      // Susun ulang urutan elemen di DOM berdasarkan array yang sudah disortir
      container.appendChild(racer.domNodes.row);
    });
  },
  ...
  renderStrategyUnderTrack(playerCars) {
    ...
    container.innerHTML = playerCars
      .map((car, index) => {
        ...
        <div class="driver-strategy-row">
          ...
          <div class="strat-telemetry">
            <div class="mini-stat">
              ...
              <div class="mini-bar"><div class="mini-bar-fill" style="width:${wearPct}%; background:${
          wearPct > 60 ? "#00cc66" : wearPct > 30 ? "#ffaa00" : "#ff4444"
        }"></div></div>
              <span class="mini-val">${wearPct}%</span>
            </div>
            <div class="mini-stat">
              <span class="mini-label">FUEL</span>
              <div class="mini-bar"><div class="mini-bar-fill" style="width:${fuelPct}%; background:${
          fuelPct > 30 ? "#00aaff" : "#ff4444"
        }"></div></div>
              <span class="mini-val">${fuelPct}%</span>
            </div>
            <div class="mini-stat">
              <span class="mini-label">ERS</span>
              <div class="mini-bar"><div class="mini-bar-fill" style="width:${ersPct}%; background:${
          ersPct > 40 ? "#ffcc00" : "#ff6600"
        }"></div></div>
          ...
          <div class="strat-controls">
            <div class="control-group-inline">
              ...
              <button class="btn-ctrl ${
                car.drivingStyle === "fast" ? "active-red" : ""
              }" onclick="Engine.setDrivingStyle('fast', this, ${index})">PUSH</button>
              <button class="btn-ctrl ${
                car.drivingStyle === "stable" ? "active-green" : ""
              }" onclick="Engine.setDrivingStyle('stable', this, ${index})">STD</button>
              <button class="btn-ctrl ${
                car.drivingStyle === "slow" ? "active-blue" : ""
              }" onclick="Engine.setDrivingStyle('slow', this, ${index})">SAVE</button>
            </div>
            <div class="control-group-inline">
              <span class="ctrl-label">ERS</span>
              <button class="btn-ctrl ${
                car.ersMode === "deploy" ? "active-red" : ""
              }" onclick="Engine.setERSMode('deploy', ${index})">DPL</button>
              <button class="btn-ctrl ${
                car.ersMode === "balanced" ? "active-green" : ""
              }" onclick="Engine.setERSMode('balanced', ${index})">BAL</button>
              <button class="btn-ctrl ${
                car.ersMode === "harvest" ? "active-blue" : ""
              }" onclick="Engine.setERSMode('harvest', ${index})">HRV</button>
            </div>
            <div class="control-group-inline">
              <span class="ctrl-label">PIT</span>
              <button class="btn-compound-sm" style="border-color:#ff0000" ${
                car.isPitting || car.pitRequested ? "disabled" : ""
              } onclick="Engine.pitStop(${index}, 'soft')">S</button>
              <button class="btn-compound-sm" style="border-color:#ffd700" ${
                car.isPitting || car.pitRequested ? "disabled" : ""
              } onclick="Engine.pitStop(${index}, 'medium')">M</button>
              <button class="btn-compound-sm" style="border-color:#ffffff" ${
                car.isPitting || car.pitRequested ? "disabled" : ""
              } onclick="Engine.pitStop(${index}, 'hard')">H</button>
              ${
                car.isPitting
                  ? '<span class="pit-status">PITTING...</span>'
                  : car.pitRequested
                  ? '<span class="pit-status">QUEUED</span>'
                  : ""
              }
            </div>
          </div>
  ...
  openSessionPanel(mode) {
    ...
    if (mode === "Q") {
      if (data.qualifyingComplete) {
        this.renderSessionResults(data.qualifyingGrid, "QUALIFYING FINAL GRID");
        return;
      }
      this.showRaceWeekendSummary();
      return;
  ...
  renderSessionResults(results, title) {
    ...
    results.forEach((r) => {
      ...
      const compBadge = compound
        ? `<span class="tyre-badge-sm" style="background:${compound.color}">${compound.code}</span>`
        : "-";
      const fl = r.fastestLap ? " 🟣" : "";

      html += `
        <tr class="${isP ? "player-row" : ""} ${r.dnf ? "dnf-row" : ""}">
          <td class="pos-cell ${r.pos <= 3 ? "podium" : ""}">${
        r.dnf ? "DNF" : r.pos
      }</td>
  ...
  renderRaceDebrief(results, rewards) {
    ...
    const prizeText = rewards?.money
      ? `Rp ${rewards.money.toLocaleString("id-ID")}`
      : "Rp 0";

    let pointsSummary = "";
    if (rewards?.points) {
      pointsSummary = Object.entries(rewards.points)
        .map(([id, pts]) => {
          const driver = results.find((r) => r.id === id);
          return `<span class="debrief-point">${
            driver?.code || id
          }: <strong>+${pts} pts</strong></span>`;
        })
        .join("");
    }
      ...
      eventLogHTML = `<div class="debrief-header" style="margin-bottom:15px; background: rgba(0,0,0,0.4);">
        ...
        <div style="max-height: 150px; overflow-y: auto; padding-right: 10px;">
          <ul style="list-style:none; padding:0; margin:0; font-size:11px; color:var(--text-muted); font-family:monospace; display:flex; flex-direction:column; gap:6px;">
            ${Engine.sim.teamEvents
              .map(
                (e) =>
                  `<li><strong style="color:var(--text-white)">Lap ${e.lap}:</strong> ${e.text}</li>`
              )
              .join("")}
          </ul>
        </div>
    ...
    let html = `
      ...
      <div class="debrief-header">
        <div class="debrief-rewards">
          ...
          <div class="debrief-item"><span class="debrief-label">POINTS EARNED</span><div class="debrief-points-row">${
            pointsSummary || "Tidak ada poin"
          }</div></div>
        </div>
      </div>`;
  ...
  renderStandings() {
    ...
    const driverList = Object.entries(driverPoints)
      .map(([id, pts]) => {
        ...
        const team = driver
          ? Data.teams.find((t) => t.id === driver.team_id)
          : null;
        return {
          id,
          name: driver?.full_name || id,
    ...
    driverList.forEach((d) => {
      if (!teamPointsMap[d.teamName])
        teamPointsMap[d.teamName] = {
          name: d.teamName,
          color: d.teamColor,
          pts: 0,
        };
    ...
    driverList.forEach((d, i) => {
      ...
      html += `<tr class="${isPlayer ? "player-row" : ""}">
        ...
        <td style="border-left:3px solid ${d.teamColor}; padding-left:8px">${
        d.code
      } ${d.name}</td>
        <td>${d.teamName}</td>
    ...
    teamList.forEach((t, i) => {
      html += `<tr>
        <td class="${i < 3 ? "podium" : ""}">${i + 1}</td>
        <td style="border-left:3px solid ${t.color}; padding-left:8px">${
        t.name
      }</td>
      ...
      <div class="rnd-grid">
        <div class="rnd-card">
          ...
          <div class="rnd-stats">
            ${this.renderStatBar(
              "Aero Efficiency",
              chassis?.stats?.aerodynamics?.aero_efficiency
            )}
            ${this.renderStatBar(
              "Top Speed",
              chassis?.stats?.performance?.top_speed
            )}
            ${this.renderStatBar(
              "Traction",
              chassis?.stats?.mechanical?.traction
            )}
            ${this.renderStatBar(
              "Tyre Preservation",
              chassis?.stats?.performance?.tyre_preservation
            )}
            ${this.renderStatBar(
              "Reliability",
              chassis?.stats?.reliability?.failure_resistance
            )}
          </div>
        </div>
        <div class="rnd-card">
          <div class="card-header-f1">POWER UNIT: ${
            pu?.manufacturer || "TBA"
          }</div>
          <div class="rnd-stats">
            ...
            ${this.renderStatBar(
              "ERS Output",
              pu?.stats?.hybrid_system?.ers_output
            )}
            ${this.renderStatBar(
              "Reliability",
              pu?.stats?.reliability?.failure_resistance
            )}
          </div>
        </div>
        <div class="rnd-card">
          <div class="card-header-f1">DEVELOPMENT BUDGET</div>
          <p class="rnd-budget">Rp ${(State.budget || 0).toLocaleString(
            "id-ID"
          )}</p>
  ...
  renderStatBar(label, value) {
    const v = value || 0;
    const color =
      v > 90 ? "#00ff87" : v > 75 ? "#00aaff" : v > 60 ? "#ffaa00" : "#ff4444";
  ...
  renderCalendar() {
    ...
    Data.schedules.forEach((sch) => {
      ...
      if (weekend?.race?.results) {
        ...
        if (playerResult)
          resultBadge = `<span class="calendar-result">P${playerResult.pos}</span>`;
      }

      div.innerHTML = `
        <div class="calendar-round">R${sch.round}</div>
        <div class="calendar-flag">
          <img src="https://flagsapi.com/${countryCode}/flat/48.png" alt="${
        sch.country
  ...
  showRaceDetailsModal(scheduleItem, circuitName, dateRange) {
    ...
    document.getElementById("modal-grand-prix-name").innerText =
      scheduleItem.grand_prix;
    ...
    scheduleItem.sessions.forEach((session) => {
      ...
      div.innerHTML = `<span>${session.session}</span><span>${new Date(
        session.date
      ).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      })} ${session.time_utc.substring(0, 5)} UTC</span>`;
    ...
    if (weekend && (weekend.fp || weekend.q1 || weekend.race)) {
      historyList.innerHTML = `<div class="modal-results-history"><h4>HASIL WEEKEND</h4>${this.renderWeekendHistoryHtml(
        weekend
      )}</div>`;
    }
  ...
  renderWeekendHistoryHtml(weekend) {
    const renderTop = (title, sessionData) => {
      ...
      const rows = sessionData.results
        .slice(0, 5)
        .map(
          (r) =>
            `<li class="${r.isPlayer ? "player-li" : ""}">#${r.pos} ${
              r.code
            } - ${r.time}</li>`
        )
        .join("");
    ...
    return [
      ...
      renderTop("Q2", weekend.q2),
      renderTop("Q3", weekend.q3),
  ...
  renderHorizontalCards(containerId, itemsList, targetField, callbackEvent) {
    ...
    const sortedItems = [...itemsList].sort((a, b) => {
      const priceA =
        a.price_idr || a.financials?.estimated_annual_value_idr || 0;
      const priceB =
        b.price_idr || b.financials?.estimated_annual_value_idr || 0;
    ...
    sortedItems.forEach((item) => {
      ...
      const driverSlot =
        WizardForm.driver1Id === itemId
          ...
          ? 2
          : null;
      const isAlreadySelected = isDriverSelection
        ? driverSlot !== null
      ...
      const itemPrice =
        item.price_idr || item.financials?.estimated_annual_value_idr || 0;
      const isSponsorSelection = containerId === "container-sponsor";
      const isAffordable =
        isSponsorSelection ||
        isAlreadySelected ||
        Finance.currentWizardBudget >= itemPrice;
      ...
      if (item.financials?.estimated_annual_value_idr) {
        statsHtml += `<div class="card-stat-row"><span class="card-stat-label">Dana:</span><span class="card-stat-value" style="color:var(--f1-neon)">Rp ${(
          item.financials.estimated_annual_value_idr / 1e9
      ...
      if (item.stats?.overall_performance)
      ...
      if (item.price_idr)
        costLabel = `Rp ${(item.price_idr / 1e9).toLocaleString("id-ID")}B`;
      else if (item.financials) costLabel = "KONTRAK";

      card.innerHTML = `
        <div class="selected-badge">${
          isDriverSelection && driverSlot
            ? `DRIVER ${driverSlot}`
            : "TERKONTRAK"
        }</div>
      ...
      if (!isAffordable) {
        ...
        if (priceEl && !isSponsorSelection)
          priceEl.innerText = "Anggaran Tidak Cukup";
      }
  ...
  moveWizard(direction) {
    ...
    document
      ...
      .classList.toggle("hidden", WizardForm.step === 4);
    document
      .getElementById("btn-wizard-submit")
      .classList.toggle("hidden", WizardForm.step !== 4);
  },
  ...
  canLeaveWizardStep(step) {
    if (
      step === 1 &&
      (!document.getElementById("input-team-name").value.trim() ||
        !WizardForm.sponsorId)
    ) {
      alert("Isi nama konstruktor dan pilih sponsor utama.");
      return false;
    }
    if (step === 2 && (!WizardForm.teamChiefId || !WizardForm.techChiefId)) {
      alert("Pilih Team Principal dan Technical Director.");
      return false;
    }
    if (step === 3 && (!WizardForm.puId || !WizardForm.chassisId)) {
      alert("Pilih Power Unit dan Chassis.");
      return false;
    }
    return true;
  },
  ...
  updateTopBar() {
    ...
    if (budgetEl)
      budgetEl.innerText = (State.budget || 0).toLocaleString("id-ID");
  },
  ...
  setupHQ() {
    ...
    s(
      "hq-pu",
      State.pu
        ? `${State.pu.manufacturer || ""} ${
            State.pu.architecture || ""
          }`.trim() || State.pu.name
        : "TBA"
    );
    ...
    const nextEvent = Data.schedules.find(
      (e) => e.round === State.currentRound && e.type === "race_weekend"
    );
    const completed = State.raceWeekends
      ? Object.values(State.raceWeekends).filter((w) => w.race).length
      : 0;
    const points = State.driverPoints || {};
    const pointText = State.drivers?.length
      ? State.drivers
          .map(
            (d) => `${d.driver_code || d.last_name}: ${points[d.id] || 0} pts`
          )
          .join(" | ")
      : "0";
    ...
    s(
      "hq-next-round",
      nextEvent
        ? `R${nextEvent.round} - ${nextEvent.grand_prix}`
        : "Season Complete"
    );
